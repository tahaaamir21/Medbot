...
def main():
    ...
    st.markdown('<div class="header-subtitle">Your AI-powered medical assistant. Ask any health-related question!</div>', unsafe_allow_html=True)
    ...
    if prompt:
        ...
        try: 
            ...
            # Format the answer
            answer_md = f"<b>Answer:</b><br>{result.strip()}"

            # Format the sources as a card
            sources_md = ""
            if source_documents:
                sources_md += "<div class='source-card'><b>Source Documents:</b><br>"
                for i, doc in enumerate(source_documents, 1):
                    ...
                    sources_md += f"<b>Source {i}:</b> {source}<br>{snippet}<br>"
                sources_md += "</div>"

            result_to_show = answer_md + sources_md
